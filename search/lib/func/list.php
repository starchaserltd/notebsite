...
foreach ( array_slice($_POST,2) as $param_name => $param_val) 
{
	...
	{
		case "string":
		{ ${$param_name} = filter_var($param_val, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH); break; }
		case "array":
		{ ${$param_name} = filter_var_array($param_val, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH); break; }
		case "integer":
		{ ${$param_name} = filter_var($param_val, FILTER_SANITIZE_NUMBER_INT); break; }
		case "float":
		{ ${$param_name} = filter_var($param_val, FILTER_SANITIZE_NUMBER_FLOAT); break; }
