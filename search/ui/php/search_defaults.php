...
$to_return = false;                               // keep sentinel but treat as array only when populated
function list_to_int($list) { return array_map(fn(string $id): int => (int) $id, $list); }
$gen_new_data = 0;
if (isset($_POST['get_new_search_data'])) {
    $gen_new_data = filter_var($_POST['get_new_search_data'], FILTER_VALIDATE_INT);
}
$gen_new_data = 1;
if ($gen_new_data == 1) {
    ...
    if (count($nomen_key) > 0) {
        // Ensure $to_return is an array before we start indexing (PHP 8.1 compatibility)
        if ($to_return === false) { $to_return = []; }
        ...
        /** POST PROCESSING **/
        if (isset($to_return["price"])) {
            $to_return["model"]["pricemin"] = $to_return["price"]["min"];
            $to_return["model"]["pricemax"] = $to_return["price"]["max"];
        }
