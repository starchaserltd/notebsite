...
$sel="SELECT name FROM notebro_site.nomen WHERE type=16 OR type=21 OR type=31 OR type=35";
//echo $sel;
mysqli_multi_query($con, $sel);
$result=mysqli_use_result($con);
$rand = mysqli_fetch_all($result);
$cpufreqmaxi=floatval($rand[0][0]); $cputdpmindb=floatval($rand[1][0]); $dispsizemindb=floatval($rand[2][0]); $memcapmindb=floatval($rand[3][0]);
