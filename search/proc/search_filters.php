...
if(strcmp("kMuGLmlIzCWmkNbtksAh",$_SESSION['auth'])==0)
{
	...
	if($to_search["model"])
    {
		...
		$comp_lists["model"] = search_model ($search_ids_model,$model_model, $prod_model, $fam_model,$msc_model, $regions_id, $model_minclass, $model_maxclass, $model_advclass);
	}
	...
	if($to_search["display"])
	{
		...
		$comp_lists["display"] = search_display ($display_model, $display_sizemin, $display_sizemax, $display_format, $display_hresmin, $display_hresmax, $display_vresmin, $display_vresmax, $display_surft, $display_backt, $display_touch,  $display_misc, $display_resolutions, $display_ratingmin, $display_ratingmax, $pricemin, $budgetmax, $battery_life,$display_srgb,$display_dcip,$display_hz,$display_hdr);
	}
	...
	{ 
		...
		$comp_lists["gpu"] = search_gpu ($gpu_typelist, $gpu_prod, $gpu_model, $gpu_variant, $gpu_name, $gpu_arch, $gpu_techmin, $gpu_techmax, $gpu_shadermin, $gpu_cspeedmin, $gpu_cspeedmax, $gpu_sspeedmin, $gpu_sspeedmax, $gpu_mspeedmin, $gpu_mspeedmax, $gpu_mbwmin, $gpu_mbwmax, $gpu_mtype, $gpu_maxmemmin, $gpu_maxmemmax, $gpu_sharem, $gpu_powermin, $gpu_powermax, $gpu_ldmin, $gpu_ldmax, $gpu_misc, $gpu_ratemin, $gpu_ratemax, $pricemin ,$budgetmax, $battery_life, $gpu_idlist);
	}
	...
	if($to_search["chassis"])
	{
		...
		$comp_lists["chassis"] = search_chassis ($chassis_prod, $chassis_model, $chassis_thicmin, $chassis_thicmax, $chassis_depthmin, $chassis_depthmax, $chassis_widthmin, $chassis_widthmax, $chassis_color, $chassis_weightmin, $chassis_weightmax, $chassis_made, $chassis_charger, $chassis_ports, $chassis_vports, $chassis_webmin, $chassis_webmax, $chassis_touch, $chassis_misc, $chassis_stuff, $chassis_ratemin, $chassis_ratemax, $pricemin,$budgetmax,$chassis_extra_stuff,$chassis_twoinone,$chassis_addpi);
	}
