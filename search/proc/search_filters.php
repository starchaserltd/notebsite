...
if(strcmp("kMuGLmlIzCWmkNbtksAh",$_SESSION['auth'])==0)
{
	...
	if($to_search["cpu"])
	{
		require_once("proc/cpu_search.php");
	...
	if($to_search["gpu"])
	{
		require_once("proc/gpu_search.php");
		$comp_lists["gpu"] = search_gpu ($gpu_typelist, $gpu_prod, $gpu_model, $gpu_arch, $gpu_techmin, $gpu_techmax, $gpu_shadermin, $gpu_cspeedmin, $gpu_cspeedmax, $gpu_sspeedmin, $gpu_sspeedmax, $gpu_mspeedmin, $gpu_mspeedmax, $gpu_mbwmin, $gpu_mbwmax, $gpu_mtype, $gpu_maxmemmin, $gpu_maxmemmax, $gpu_sharem, $gpu_powermin, $gpu_powermax, $gpu_misc, $gpu_ratemin, $gpu_ratemax, $pricemin ,$budgetmax, $battery_life);
	}

	if($to_search["acum"])
	{
		require_once("proc/acum_search.php");
	...
	if($to_search["war"])
	{
		require_once("proc/war_search.php");
	...
	if($to_search["hdd"])
	{
		require_once("proc/hdd_search.php");
	...
	if($to_search["shdd"])
	{
		require_once("proc/shdd_search.php");
		$comp_lists["shdd"] = search_shdd ($nr_hdd);
	}

	if($to_search["wnet"])
	{
		require_once("proc/wnet_search.php");
	...
	if($to_search["mem"])
	{
		require_once("proc/mem_search.php");
	...
	if($to_search["mdb"])
	{
		require_once("proc/mdb_search.php");
