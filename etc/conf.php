...
function table($id) { $t=explode("_",$id); $id=$t[0]; if(isset($t[1])){ $model=$t[1]; } $result=mysqli_query($GLOBALS["con"],"SELECT model FROM notebro_temp.all_conf WHERE id=$id LIMIT 1"); if($result && mysqli_num_rows($result)>0) {  $model=mysqli_fetch_row($result); mysqli_free_result($result); $t[1]=$model[0]; } else { if(isset($model)) { $result=mysqli_query($GLOBALS["con"],"SELECT best_value FROM notebro_temp.best_low_opt WHERE id_model=$model LIMIT 1"); $id=mysqli_fetch_row($result); mysqli_free_result($result);  $t[0]=$id[0]; } else { $t=NULL; } }  return $t; }
