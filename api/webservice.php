		...
		if(intval($hits_left)>0)
		{
			...
			if($method!==NULL && $method!=="")
			{
				switch($method)
				{
					case "get_model_info":
					{
						$response->code=26; $response->message="Valid method.";$response->daily_hits_left=$hits_left; if(isset($param['model_id'])&&!empty($param['model_id'])) { $param['model_id']=explode(",",$param['model_id'])[0];}
					...
					case "get_model_info_all":
					{
						...
						if(isset($idmodel[0]))
						{
							...
							while( $row=mysqli_fetch_assoc($resulti))
							{
								...
								$object_addr->model_resources=comp_details("model_res",$idmodel[0]);
								//Components
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["cpu"]) as $el) { $object_addr->cpu->{$j}=comp_details("cpu",$el); if(intval($row["cpu"])==intval($object_addr->cpu->{$j}["id"])){ $selected=$j; } unset($object_addr->cpu->{$j}["id"]); $j++; } $object_addr->cpu->selected=$selected;
								$integrated=0; $selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["gpu"]) as $el) { $object_addr->gpu->{$j}=comp_details("gpu",$el); if(intval($row["gpu"])==intval($object_addr->gpu->{$j}["id"])){ $selected=$j; } foreach($object_addr->cpu as $key=>$el){ if(intval($el["integrated_video_id"])==intval($object_addr->gpu->{$j}["id"])){ $object_addr->cpu->$key["integrated_video_id"]=$j; } } if(intval($object_addr->gpu->{$j}["typegpu"])==0) { $integrated=1; } unset($object_addr->gpu->{$j}["typegpu"]); unset($object_addr->gpu->{$j}["id"]); $j++; } $object_addr->gpu->selected=$selected; if(!$integrated){ foreach($object_addr->cpu as $key=>$el){ $object_addr->cpu->$key["integrated_video_id"]=NULL; } }
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["display"]) as $el) { $object_addr->display->{$j}=comp_details("display",$el); if(intval($row["display"])==intval($object_addr->display->{$j}["id"])){ $selected=$j; } unset($object_addr->display->{$j}["id"]); $j++; } $object_addr->display->selected=$selected;
								$selected=NULL; foreach(explode(",",$object_addr->model_resources["hdd"]) as $el) { $data=comp_details("hdd",$el); $j=intval($data["id"]); unset($data["id"]); $object_addr->primary_storage->{$j}=$data; if(intval($row["hdd"])==$j){ $selected=$j; } $j++; } $object_addr->primary_storage->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["shdd"]) as $el) { $object_addr->secondary_storage->{$j}=comp_details("hdd",$el); if(intval($row["shdd"])==intval($object_addr->secondary_storage->{$j}["id"])){ $selected=$j; } unset($object_addr->secondary_storage->{$j}["id"]); $j++; } $object_addr->secondary_storage->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["mdb"]) as $el) { $object_addr->motherboard->{$j}=comp_details("mdb",$el); if(intval($row["mdb"])==intval($object_addr->motherboard->{$j}["id"])){ $selected=$j; } unset($object_addr->motherboard->{$j}["id"]); $j++; } $object_addr->motherboard->selected=$selected;							
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["mem"]) as $el) { $object_addr->memory->{$j}=comp_details("mem",$el); if(intval($row["mem"])==intval($object_addr->memory->{$j}["id"])){ $selected=$j; } unset($object_addr->memory->{$j}["id"]); $j++; } $object_addr->memory->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["odd"]) as $el) { $object_addr->optical_drive->{$j}=comp_details("odd",$el); if(intval($row["odd"])==intval($object_addr->optical_drive->{$j}["id"])){ $selected=$j; } unset($object_addr->optical_drive->{$j}["id"]); $j++; } $object_addr->optical_drive->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["acum"]) as $el) { $object_addr->battery->{$j}=comp_details("acum",$el); if(intval($row["acum"])==intval($object_addr->battery->{$j}["id"])){ $selected=$j; } unset($object_addr->battery->{$j}["id"]); $j++; } $object_addr->battery->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["chassis"]) as $el) { $object_addr->chassis->{$j}=comp_details("chassis",$el); if(intval($row["chassis"])==intval($object_addr->chassis->{$j}["id"])){ $selected=$j; } unset($object_addr->chassis->{$j}["id"]); $j++; } $object_addr->chassis->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["wnet"]) as $el) { $object_addr->wireless->{$j}=comp_details("wnet",$el); if(intval($row["wnet"])==intval($object_addr->wireless->{$j}["id"])){ $selected=$j; } unset($object_addr->wireless->{$j}["id"]); $j++; } $object_addr->wireless->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["warranty"]) as $el) { $object_addr->warranty->{$j}=comp_details("war",$el); if(intval($row["war"])==intval($object_addr->warranty->{$j}["id"])){ $selected=$j; } unset($object_addr->warranty->{$j}["id"]); $j++; } $object_addr->warranty->selected=$selected;
								$selected=NULL; $j=0; foreach(explode(",",$object_addr->model_resources["sist"]) as $el) { $object_addr->operating_system->{$j}=comp_details("sist",$el); if(intval($row["sist"])==intval($object_addr->operating_system->{$j}["id"])){ $selected=$j; } unset($object_addr->operating_system->{$j}["id"]); $j++; } $object_addr->operating_system->selected=$selected;
								unset($object_addr->model_resources["cpu"]); unset($object_addr->model_resources["display"]); unset($object_addr->model_resources["mem"]); unset($object_addr->model_resources["hdd"]); unset($object_addr->model_resources["shdd"]);
								unset($object_addr->model_resources["gpu"]); unset($object_addr->model_resources["wnet"]); unset($object_addr->model_resources["odd"]); unset($object_addr->model_resources["mdb"]); unset($object_addr->model_resources["chassis"]);
								unset($object_addr->model_resources["acum"]); unset($object_addr->model_resources["warranty"]); unset($object_addr->model_resources["sist"]); unset($object_addr->model_resources["id"]);
								//OTHER CONFIG INFO
