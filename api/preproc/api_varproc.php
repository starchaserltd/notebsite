	...
	switch($v)
	{
		...
		{		
			...
			{	
				...
				{ 
					...
					$result = mysqli_query($GLOBALS['con'], "SELECT `id` FROM `".$GLOBALS['global_notebro_db']."`.`MODEL` WHERE `MODEL`.`id` IN (".$param['model_id'].")".$from_date." LIMIT ".$nr_max_models);
		...
		case 'cpu':
		{
			if (isset($param['cpu_name']) && !empty($param['cpu_name']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT name FROM `".$GLOBALS['global_notebro_site']."`.nomen WHERE type = 11 ORDER BY type ASC");
				...
				if($cpu_name = mysqli_fetch_row(mysqli_query($GLOBALS['con'],"SELECT model FROM `".$GLOBALS['global_notebro_db']."`.CPU where model like '%".$param['cpu_name']."%' limit 1")))
		...
		case 'display' :
		{
			...
			if (isset($param['display_type'])&& !empty($param['display_type']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT name FROM `".$GLOBALS['global_notebro_site']."`.`nomen` WHERE name LIKE '%".$param['display_type']."%' AND type=10");
				if($result && mysqli_num_rows($result)>0)
				{
		...
		case 'hdd' :
		{
			...
			if (isset($param['first_hdd_type'])&& !empty($param['first_hdd_type']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT name FROM `".$GLOBALS['global_notebro_site']."`.`nomen` WHERE type=54 AND name LIKE '%".$param['first_hdd_type']."%'");
				if($result && mysqli_num_rows($result)>0)
				{
		...
		case 'gpu' :
		{
			if (isset($param['gpu_name'])&& !empty($param['gpu_name']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT `name` FROM `".$GLOBALS['global_notebro_site']."`.`nomen` WHERE `type` = 12 ORDER BY `type` ASC");
				...
				$sql = "SELECT `name` AS `model` FROM `".$GLOBALS['global_notebro_db']."`.`GPU` WHERE `name` '%".$param['gpu_name']."%' ";
		...
		case 'wnet' :
		{
			if (isset($param['wireless_name'])&& !empty($param['wireless_name']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT DISTINCT prod FROM `".$GLOBALS['global_notebro_db']."`.WNET WHERE 1=1");
				...
				if($result=mysqli_fetch_row(mysqli_query($GLOBALS['con'],"SELECT model FROM `".$GLOBALS['global_notebro_db']."`.WNET where model like '%".$param['wireless_name']."%' limit 1")))
		...
		case 'odd' :
		{
			if (isset($param['odd_type'])&& !empty($param['odd_type']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT name FROM `".$GLOBALS['global_notebro_db']."`.`nomen` WHERE name LIKE '%".$param['odd_type']."%' AND type=52");
				if($result && mysqli_num_rows($result)>0)
				{
		...
		case 'sist' :
		{
			if (isset($param['opsist'])&& !empty($param['opsist']) && !$abort)
			{
				...
				$result = mysqli_query($GLOBALS['con'], "SELECT name FROM `".$GLOBALS['global_notebro_site']."`.`nomen` WHERE name LIKE '%".$param['opsist']."%' AND type=25 LIMIT 1");
				if($result && mysqli_num_rows($result)>0)
				{
