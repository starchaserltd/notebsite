...
function comp_details($comp,$id)
{
	switch($comp)
	{
		...
		{
			$sel="SELECT `id` FROM `".$global_notebro_db."`.`MODEL` WHERE `p_model`=(SELECT `p_model` FROM `".$global_notebro_db."`.`MODEL` WHERE `id`='".$id."' LIMIT 1)";
		...
		case "cpu":
		{
			$sel="SELECT `id`,`prod`,`model`,`tech` AS `lithography`,`cache`,ROUND(`clocks`,2) AS `base_speed`,ROUND(`maxtf`,2) AS `boost_speed`,`cores`,ROUND(`tdp`,0) AS `tdp`,`msc` AS `other_info`,ROUND(`rating`,1) AS `rating`,`gpu` AS `integrated_video_id` FROM `".$global_notebro_db."`.`CPU` WHERE `id`='".$id."'";
			...
			if(have_results($some_query))
			{
				...
				$some_query_2=mysqli_query($GLOBALS['con'],"SELECT `prod`,`model_name` FROM `".$global_notebro_db."`.`GPU` WHERE `id`='".$res['integrated_video_id']."'");
		...
		case "display":
		{
			$sel="SELECT id,size,hres as horizontal_resolution,vres as vertical_resolution,backt as type,sRGB,touch,msc as other_info FROM `".$global_notebro_db."`.DISPLAY WHERE id=".$id."";
		...
		case "mem":
		{
			$sel="SELECT id,cap as size,freq as speed,type FROM `".$global_notebro_db."`.MEM WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
			return $res; break;
		}
		case "hdd":
		{
			$sel="SELECT id,model,cap,rpm,readspeed as read_speed FROM `".$global_notebro_db."`.HDD WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
		...
		case "wnet":
		{		
			$sel="SELECT id,model,speed,msc as other_info FROM `".$global_notebro_db."`.WNET WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
			return $res; break;
		}
		case "odd":
		{
			$sel="SELECT id,type,msc as other_info FROM `".$global_notebro_db."`.ODD WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
		...
		case "mdb":
		{
			$sel="SELECT id,ram as ram_slots,netw as lan_card,hdd as storage_slots,msc as other_info FROM `".$global_notebro_db."`.MDB WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
		...
		case "chassis":
		{
			$sel="SELECT `id`,ROUND(`thic`/10,2) AS `height_cm`, ROUND(`thic`*0.0393701,2) AS `height_inch`,ROUND(`depth`/10,2) AS `depth_cm`, ROUND(`depth`*0.0393701,2) AS `depth_inch`, ROUND(`width`/10,2) AS `width_cm`, ROUND(`width`*0.0393701,2) AS `width_inch`,ROUND(`weight`,2) AS `weight_kg`, ROUND(`weight`*2.20462262,2) AS `weight_lb`, `color` AS `colors`,`made` AS `build_materials`,`pi` AS `peripheral_interfaces`, `vi` AS `video_interfaces`, `web` AS `webcam_mp`,`keyboard` AS `keyboard_type`,`charger`,`msc` AS `other_info` FROM ``".$global_notebro_db."``.`CHASSIS` WHERE `id`=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
			return $res; break;
		}
		case "acum":
		{
			$sel="SELECT id,cap as capacity,tipc as cell_type,msc as other_info FROM `".$global_notebro_db."`.ACUM WHERE id=".$id."";
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
			return $res; break;
		}	
		case "war":
		{	
			$sel="SELECT id,years,prod as type_short,msc as type_long FROM `".$global_notebro_db."`.WAR WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
			return $res; break;
		}
		case "sist":
		{
			$sel="SELECT id,CONCAT(sist,' ',type,' ',vers) as name FROM `".$global_notebro_db."`.SIST WHERE id=".$id.""; 
			$res=array(); $res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'], $sel));
			return $res; break;
		}
		default:
