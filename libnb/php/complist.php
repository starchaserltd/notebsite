	...
	for ($i=0;$i<9;$i++)
	{
		if(isset($_SESSION['conf'.$i]['id']))
		{
			...
			echo '<tbody><tr id="comprow'.$_SESSION['conf'.$i]['id'].'"class="items" style="background:#fff;"><td class="comparecell1"><div class="checkbox" style="margin:0px; width:0px;"><input type="checkbox" onclick="cchecks('."'".$_SESSION['conf'.$i]['id']."'".')" class="css-checkbox sme" id="checkbox'.$_SESSION['conf'.$i]['id'].'" '.$checked.' /><label style="font-weight:normal;min-height:16px;" for="checkbox'.$_SESSION['conf'.$i]['id'].'" class="css-label sme depressed"></label></div></td>
				<td class="text-center comparecell3"><a  href="'.$web_address.'?model/model.php?conf='.$_SESSION['conf'.$i]['id'].'" class="comparename"><img class="img-responsive" src="res/img/models/thumb/t_'.$_SESSION['conf'.$i]['img'].'" alt="'.$_SESSION['conf'.$i]['img'].'"></a></td>
				<td class="text-center comparecell2" ><a  href="'.$web_address.'?model/model.php?conf='.$_SESSION['conf'.$i]['id'].'" class="comparename">'.$_SESSION['conf'.$i]['name'];
