...
function get_best_low($cons,$current_ex_region,$model_id)
{
	...
	$sql="SELECT * FROM `".$GLOBALS['global_notebro_sdb']."`.`best_low_opt` WHERE `id_model`=CONCAT('p_',(SELECT `pmodel` FROM `".$GLOBALS['global_notebro_sdb']."`.`m_map_table` WHERE `model_id`=".$model_id." LIMIT 1),'_".strval($cur_p_region)."') LIMIT 1";
	...
	{
		$model_search=true;
		$sql="SELECT * FROM `".$GLOBALS['global_notebro_sdb']."`.`best_low_opt` WHERE `id_model` LIKE '".$model_id."_".strval($cur_p_region)."'";
		$best_low=mysqli_fetch_assoc(mysqli_query($cons,$sql));
		if(!(isset($best_low["best_value"])&&$best_low["best_value"]!=""&&$best_low["best_value"]!=NULL))
		{
			$sql="SELECT * FROM `".$GLOBALS['global_notebro_sdb']."`.`best_low_opt` WHERE `id_model` LIKE '".$model_id."%'";
			$best_low=mysqli_fetch_assoc(mysqli_query($cons,$sql));
		}
	}
...
function model_in_region($cons,$model_id,$exchcode)
{
	...
	$sql="SELECT `regions` FROM `".$GLOBALS['global_notebro_sdb']."`.`ex_map_table` WHERE `ex`='".$exchcode."' LIMIT 1";
	$result=mysqli_query($cons,$sql);
	if($result&&mysqli_num_rows($result)>0)
	{
		$columns="IFNULL(`".implode("`,''),IFNULL(`",explode(",",mysqli_fetch_array($result)[0]))."`,'')";
		$sql="SELECT GROUP_CONCAT(".$columns.") as `result` FROM `".$GLOBALS['global_notebro_sdb']."`.`m_map_table` WHERE `model_id`='".$model_id."' LIMIT 1";
		$result=mysqli_query($cons,$sql);
		if($result&&mysqli_num_rows($result)>0)
		{
