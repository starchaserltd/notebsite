...
if($q>=0)
{
	...
	while($r = mysqli_fetch_assoc($result)) 
	{
		...
		{ $rows[0]['bat']=((floatval($rows[0]["size"])*0.10)+(pow((intval($rows[0]["hres"])*intval($rows[0]["vres"])),0.6)*0.0002))*0.9; }
		else
		{ $rows[0]['bat']=((floatval($rows[0]["size"])*0.10)+(pow((intval($rows[0]["hres"])*intval($rows[0]["vres"])),0.6)*0.0002))*1; }
