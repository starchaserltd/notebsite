...
if(!($result && mysqli_num_rows($result)>0)){ $result=mysqli_query($con,"SELECT GROUP_CONCAT(`notebro_buy`.`SELLERS`.`id`) AS `id` FROM `notebro_buy`.`SELLERS` WHERE `region` IN (".$buy_regions.") AND id IN (2,3,4,5,6,7,8,9,10) ".$excluded_sellers." ORDER BY priority DESC"); }
if(!($result && mysqli_num_rows($result)>0)){ $result=mysqli_query($con,"SELECT GROUP_CONCAT(`notebro_buy`.`SELLERS`.`id`) AS `id` FROM `notebro_buy`.`SELLERS` WHERE `region` IN (1,2) AND id IN (2,3,4,5,6,7,8,9,10) ".$excluded_sellers." ORDER BY priority DESC"); }
$sellers=array(); if($result && mysqli_num_rows($result)>0){ $sellers=array_unique(explode(",",mysqli_fetch_assoc($result)["id"])); }
