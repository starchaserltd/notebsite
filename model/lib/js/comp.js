// GLOBALS
var phpData,template,templateHidden,allTablesShown = false;
...
function modelGen(data)
{
	...
	function ratingRow()
	{
		...
		{
			var coloredCount = Math.round(parseInt(rating) / 10);
			var result = new Array(10)
				.fill(undefined)
				.map(function (_, index)
				{
					return `<div class="compare__ratingBlock ${
        ...
        <td class="compare__data">
          <div>${RATING} / 100</div>
          <div class="compare__rating">${createRatingBlocks(RATING)}</div>
        </td>
      `;
		});
		return makeRow(cols);
	...
	function batteryRow()
	{
		...
		data.forEach(function (laptop)
		{
			...
			BATLIFE=hourminutes(parseFloat(BATLIFE)*0.96)+" - "+hourminutes(parseFloat(BATLIFE)*1.03);
...
phpData = (function ()
{
	var parsedData = Object.values(window.array_var_new)
		.map(function (item)
		{
			var result = {
				...item,
...
(function ()
{
	...
	{
		nameAffixTrigger = nameAffixHookElement.getBoundingClientRect()
