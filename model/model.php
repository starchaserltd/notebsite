	...
	<div class="container-fluid headerback" style="margin-right:0px;padding-right: 0px; padding-left: 0px;">
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-lg-offset-0">
						<div class="model-main-title">
							<h2 class="modelHeader bold-font">
								...
								$mfam = $model_data["fam"] . $model_data["subfam"];
								$mmodel = $model_data["model"];
								...
								if (array_search("1", $mregion_id) === FALSE) {
									...
									$buy_regions = $model_data['regions'];
								} else {
									$mregion = "";
									$buy_regions = 0;
								}
				...
				?> <div class="row top-container">
					<div class="col-lg-5 col-xl-5">
						<?php if (isset($imglist["img_1"])) {
							if (!$js_ref_image) {
								echo "<script>ref_model_image='" . $web_address . "res/img/models/" . $imglist["img_1"] . "';</script>";
								$js_ref_image = true;
							} ?>
						...
						if ($imglist["img_2"]) {
							if (!$js_ref_image) {
								echo "<script>ref_model_image='" . $web_address . "res/img/models/" . $imglist["img_2"] . "';</script>";
								$js_ref_image = true;
							} ?>
						...
						if ($imglist["img_3"]) {
							if (!$js_ref_image) {
								echo "<script>ref_model_image='" . $web_address . "res/img/models/" . $imglist["img_3"] . "';</script>";
								$js_ref_image = true;
							} ?>
						...
						if ($imglist["img_4"]) {
							if (!$js_ref_image) {
								echo "<script>ref_model_image='" . $web_address . "res/img/models/" . $imglist["img_4"] . "';</script>";
								$js_ref_image = true;
							} ?>
								...
								<div class="buy">
									<div class="officialSiteContainer rating">
										...
										<div class="officialSite officialSiteLogo">
											...
											if (isset($show_vars["link2"]) && $show_vars["link2"] && isset($usertag) && $usertag != "" && $usertag != "noref") {
												$include_aff_gen = true;
												$function_replay = null;
												$_POST["usertag"] = $usertag;
												$_POST["links"] = $show_vars["link"];
												$_POST["sellers"] = $show_vars["link2"];
											...
											if ($aff_link && isset($aff_link[0]) && $aff_link[0] != NULL && $aff_link[0] != "") {
												$show_vars["link"] = $aff_link[0];
											}
											?>
									...
									<div class="dropdown actionButton">
										<button id="dLabel" class="btn btn-primary btn-lg addtocpmp buyBtn bold-font" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-ref="<?php if (isset($usertag) && $usertag != "") {
																																																																																															...
																																																																																															echo "";
																																																																																														} ?>" data-target="buylist-0" data-price="0" data-idmodel="<?php echo $idmodel; ?>" data-mprod="<?php echo $mprod; ?>" data-idmodel="<?php echo $idmodel; ?>" data-buyregions="<?php echo $buy_regions; ?>" data-lang="<?php echo $lang; ?>" data-idcpu="" data-idgpu="" data-iddisplay="" data-pmodel="<?php echo $p_model; ?>" onclick="get_buy_list(this);">
											Buy now
									...
									<div class="d-flex actionButtonsContainer">
										...
										<div class="dropdown config actionButton">
											...
											<ul class="dropdown-menu configDropdown" aria-labelledby="dLabel">
												<li class="configOptions" id="best_value_id" <?php if (!isset($best_low["best_value"]) || (isset($best_low["best_value"]) && $best_low["best_value"] == "")) {
																																				echo 'style="display:none;"';
																																			} ?> onmousedown="OpenPage('<?php echo "model/model.php?conf=" . $best_low["best_value"] . "&ex=" . $exchcode . ""; ?>',event);">Best Value</li>
												<li class="configOptions" id="best_performance_id" <?php if (!isset($best_low["best_performance"]) || (isset($best_low["best_performance"]) && $best_low["best_performance"] == "")) {
																																							echo 'style="display:none;"';
																																						} ?> onmousedown="OpenPage('<?php echo "model/model.php?conf=" . $best_low["best_performance"] . "&ex=" . $exchcode . ""; ?>',event);">Max Performance</li>
												<li class="configOptions" id="lowest_price_id" <?php if (!isset($best_low["lowest_price"]) || (isset($best_low["lowest_price"]) && $best_low["lowest_price"] == "")) {
																																					echo 'style="display:none;"';
																																				} ?> onmousedown="OpenPage('<?php echo "model/model.php?conf=" . $best_low["lowest_price"] . "&ex=" . $exchcode . ""; ?>',event);">Lowest Price</li>
											</ul>
				...
				<div class="container bg-white modelContainer pb-0">
					<div class="row accordion" id="specificationsAccordion">
									...
									<div class="row modelwidht">
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
											...
											<div class="row hidedis hideall detaliicomp" style="display:none">
												...
												<div class="col-md-7 col-xs-7 col-7 col-sm-7 col-lg-7 rows">
													<div id="progress-display-mobile" class="progress-display"></div>
													<script>
														PROGRESSBAR.create({
															target: "#progress-display-mobile",
															min: 0,
															max: 100,
															value: 50,
															maxWidth: "175px"
						...
						<div id="secondCol" class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
									...
									<div class="row modelwidht">
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
											...
											<div class="row hidedis hideall detaliicomp" style="display:none">
												...
												<div class="col-md-7 col-xs-7 col-7 col-sm-7 col-lg-7 rows">
													<div id="progress-display-desktop" class="progress-display"></div>
													<script>
														PROGRESSBAR.create({
															target: "#progress-display-desktop",
															min: 0,
															max: 100,
															value: 50,
															maxWidth: "175px"
									...
									<div class="row modelwidht">
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
											<div class="row detaliicomp">
												...
												<div class="col-md-7 col-xs-7 col-7 col-sm-7 col-lg-7 rows">
													<span>
														<?php $msctext = show_msc($idmodel);
														$msc_has_content = false;
														if (isset($msctext)) {
															if (isset($msctext["p_model"]["com"]) && $msctext["p_model"]["com"] != NULL) {
																echo $msctext["p_model"]["com"] . " ";
																$msc_has_content = true;
															}
															if (isset($msctext["p_model"]["src"]) && $msctext["p_model"]["src"] != NULL) {
																$src_msc_parts = array();
																$src_msc_parts = explode(" ", $msctext["p_model"]["src"]);
																$k = 0;
																foreach ($src_msc_parts as $val) {
																	$k++;
																	echo "<a target='_blank' href='" . $val . "'>[" . $k . "]</a>";
																}
															} ?>
													</span>
													<span id="model_msc">
													<?php if (isset($msctext["model_msc"]) && strlen($msctext["model_msc"]) > 0) {
																echo $msctext["model_msc"];
																$msc_has_content = true;
															}
														} ?>
													</span>
							...
							<?php if (!(isset($msctext) && $msc_has_content)) {
								echo "<script>document.getElementById('model_msc_card').style.display='none';</script>";
							} ?>
						</div>
				...
				<div class="container bg-transparent modelContainer reviewsContainer px-0">
					<div class="row no-gutters">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 d-md-flex ListTitle">
							<h4><?php if (!($nr_int_reviews + $nr_nb_reviews)) {
										echo "No ";
									} ?>Reviews</h4>
								...
								<ul class="list-group ListElem">
									<?php foreach (array_merge($nb_reviews, $int_reviews) as $el) { ?>
										<li class="list-group-item border-top-0 border-right-0 border-left-0">
											<a style="cursor: pointer;" class="d-flex justify-content-between align-items-center" <?php if ($el["notebreview"] == 1) { ?> onmousedown="OpenPage('<?php echo $el["link"]; ?>',event);"> <?php } else { ?> href="<?php echo $el["link"]; ?>" target="blank"> <?php } ?>
											<?php
											if ($el["notebreview"] == 1) { ?> <img class="review-image" src="model/lib/images/review-noteb.svg" alt="Noteb Review" /> <?php } else {
																																																																								if (intval($el["video"]) == 1) { ?> <img class="review-image" src="model/lib/images/review-video.svg" alt="Video Review" /> <?php  } else { ?> <img class="review-image" src="model/lib/images/review-write.svg" alt="Write Review" /> <?php } ?>
											<?php } ?>
											<div class="container">
												<div class="row">
													<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 d-md-flex justify-content-between align-items-center">
														...
														<span class="btn btn-md btn-outline-primary">
															<?php if (intval($el["video"]) == 1) { ?> Watch Video <?php } else { ?> Read More <?php } ?>
														</span>
													</div>
												</div>
											</div>
		...
		<div class="row">
			...
			<script>
				...
				$.getScript("model/lib/js/model.js", function() {
					$.getScript("lib/js/owl.carousel.min.js", function() {
						getScripts(["model/lib/js/model_post.js", "model/lib/js/sim_model.js"], function() {});
					});
				});
			</script>
	<?php
